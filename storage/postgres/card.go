...
func (r *CardRepository) CreateCard(ctx context.Context, req *pb.CreateCardReq) (*pb.CreateCardRes, error) {
    query := `
        INSERT INTO cards (user_id, card_number, expiration_date, security_code)
        VALUES ($1, $2, $3, $4)
        RETURNING id
    `
    var cardID string
    err := r.Db.QueryRowContext(ctx, query, req.UserId, req.CardNumber, req.ExpirationDate, req.SecurityCode).Scan(&cardID)
    if err != nil {
